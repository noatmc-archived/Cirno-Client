    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        run {
            when (modeValue.get().toLowerCase()) {
                ...
                "matrix" -> {
                    if (thePlayer.ticksExisted % 5 == 0) {
                        thePlayer.motionY = thePlayer.motionY * 1.01
                    }
                }
                "smoothvanilla" -> {
                ...
                "jetpack" -> if (mc.gameSettings.keyBindJump.isKeyDown) { //                   mc.effectRenderer.spawnEffectParticle(EnumParticleTypes.FLAME.getParticleID(), thePlayer.posX, thePlayer.posY + 0.2, thePlayer.posZ, -thePlayer.motionX, -0.5, -thePlayer.motionZ)
